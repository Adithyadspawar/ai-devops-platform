...
from .sentry_integration import init_sentry, wrap_app_with_sentry
import uvicorn
import os
...
# If Sentry enabled, wrap middleware (done after app creation)
...
def api_create_order(payload: OrderCreate):
    try:
        o = create_order(payload.customer_name, payload.items, payload.total_amount)
        return o
...
def api_update_order(order_id: int, payload: OrderUpdate):
    o = update_order(order_id, status=payload.status, items=payload.items, total_amount=payload.total_amount)
    if not o:
        raise HTTPException(status_code=404, detail="order not found")
    return o
...
# small metrics endpoint for Prometheus
@app.get("/metrics")
def metrics():
    # Simple metrics: count of orders
    conn = get_conn()
